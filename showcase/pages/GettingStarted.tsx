...
export const GettingStarted: React.FC = () => {
  return (
    <div className="max-w-4xl">
      <h1 className="text-3xl font-bold text-gray-900 mb-3">快速开始</h1>
      ...
      <div className="bg-accent-50 border border-accent-200 rounded-xl p-6 mb-10">
        <h3 className="text-base font-semibold text-accent-700 mb-4">特性</h3>
      ...
      <StepItem step={1} title="安装组件库">
        <p className="text-gray-500 mb-4">在主项目中添加组件库依赖：</p>
        <CodeBlock
          code={`# 方式一：相对路径引用
...
cd ../xc-sealseek-extension-sycm && npm link guanshu-component-library`}
      ...
      <StepItem step={2} title="引入样式">
        <p className="text-gray-500 mb-4">在入口文件中引入组件库样式：</p>
        <CodeBlock
...
import 'guanshu-component-library/style';`}
      ...
      <StepItem step={3} title="配置主题">
        <p className="text-gray-500 mb-4">使用 Ant Design ConfigProvider 应用主题：</p>
        <CodeBlock
  ...
  return (
    <ConfigProvider theme={theme} prefixCls={prefix}>
      ...
      <StepItem step={4} title="使用组件">
        <p className="text-gray-500 mb-4">导入并使用组件：</p>
        <CodeBlock
          code={`import {
  StatisticCard,
  FilterBar,
  ...
  return (
    <ErrorBoundary>
      ...
      <div className="mt-10 p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
        <h3 className="text-base font-semibold text-gray-900 mb-4">常用命令</h3>
        <div className="space-y-3 text-sm">
          <div className="flex items-center">
            <code className="w-44 text-accent-600 font-mono">npm run dev</code>
            <span className="text-gray-500">启动 Showcase 开发服务</span>
          </div>
          <div className="flex items-center">
            <code className="w-44 text-accent-600 font-mono">npm run build:lib</code>
            <span className="text-gray-500">构建组件库</span>
          </div>
          <div className="flex items-center">
            <code className="w-44 text-accent-600 font-mono">npm run build</code>
            <span className="text-gray-500">构建 Showcase 文档</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
        <h3 className="text-base font-semibold text-gray-900 mb-4">项目结构</h3>
        <CodeBlock
          code={`src/
├── basic/          # 基础组件 (6)
├── layout/         # 布局组件 (3)
